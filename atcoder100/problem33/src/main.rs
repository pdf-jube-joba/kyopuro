...
fn score_max(wall: &[Vec<bool>]) -> usize {
    ...
    let surr = |pos: (usize, usize)|{
        vec![
            if 0 < pos.0 && !wall[pos.0 -1][pos.1] {Some((pos.0 -1, pos.1))} else {None},
            if pos.0 < h-1 && !wall[pos.0 +1][pos.1] {Some((pos.0 +1, pos.1))} else {None},
            if 0 < pos.1 && !wall[pos.0][pos.1 -1] {Some((pos.0, pos.1 -1))} else {None},
            if pos.1 < w-1 && !wall[pos.0][pos.1 +1] {Some((pos.0, pos.1 +1))} else {None},
        ].into_iter().flatten()
    };
