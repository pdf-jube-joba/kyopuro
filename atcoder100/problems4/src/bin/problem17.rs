...
fn now_stack(p: &[(usize, usize)], s: &[usize]) -> Vec<Option<usize>> {
    ...
    let mut index = 0;
    for i in 0..8 {
        if let Some((_, y)) = p.iter().find(|(x, _y)| *x == i) {
            vec.push(Some(*y));
        } else if index < s.len() {
...
fn ok(s: &[Option<usize>], p: usize) -> bool {
    let p = if let Some((i, x)) =
        s.iter()
            .enumerate()
            .find_map(|(i, x)| if x.is_none() { Some((i, p)) } else { None })
    {
        (i, x)
    } else {
        (s.len(), p)
    ...
    let s: Vec<(usize, usize)> = s
        ...
        .collect();
    if s.iter().map(|(x, _)| *x).any(|x| x == p.0) {
        return false;
    }
    if s.iter().map(|(_, x)| *x).any(|x| x == p.1) {
        return false;
    }
    if s.iter()
