...
fn to_bool(vec: &[usize]) -> Vec<bool> {
    (0..vec.len()).map(|i| match vec[i] {0 => false, 1 => true, _ => unreachable!()}).collect()
}
...
fn min_length(w_wall: &[Vec<bool>], h_wall: &[Vec<bool>]) -> usize {
    ...
    let surr = |(y, x): (usize, usize)|{
        vec![
            if 0 < y && !h_wall[y-1][x] {Some((y-1, x))} else {None}, //up
            if y < h-1 && !h_wall[y][x] {Some((y+1, x))} else {None}, //down
            if 0 < x && !w_wall[y][x-1] {Some((y, x-1))} else {None}, //left
            if x < w-1 && !w_wall[y][x] {Some((y, x+1))} else {None} //right
        ].into_iter().flatten()
    };
    ...
    match check[h-1][w-1] {Some(len) => len, None => 0}
...
mod tests {
    ...
    fn min_length_test_1(){
        let w_wall = vec![
            ...
            vec![0],
            vec![1],
        ].into_iter().map(|vec| to_bool(&vec)).collect::<Vec<Vec<bool>>>();
        let h_wall = vec![
            vec![0,1],
            vec![1,0],
        ].into_iter().map(|vec| to_bool(&vec)).collect::<Vec<Vec<bool>>>();;
