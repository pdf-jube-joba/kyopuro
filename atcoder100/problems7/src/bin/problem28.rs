...
mod tests {
    ...
    #[test]
    fn test_BFS_1() {
        let vec = vec![vec![1, 3], vec![3], vec![], vec![2]];
        let expect = vec![Some(0), Some(1), Some(2), Some(1)];
        let result = BFS(&vec);
        assert_eq!(result, expect);
    }
    #[test]
    fn test_BFS_2() {
        let vec = vec![vec![1, 3], vec![4], vec![4, 5], vec![], vec![3], vec![5]];
        let expect = vec![Some(0), Some(1), None, Some(1), Some(2), None];
        let result = BFS(&vec);
        assert_eq!(result, expect);
    }
}
